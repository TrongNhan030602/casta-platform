...
import {
  productCreateSchema,
  productUpdateSchema,
} from "@/validations/enterprise/product";
...
const ProductForm = ({
  ...
  const schema = mode === "update" ? productUpdateSchema : productCreateSchema;
      ...
      Object.entries(defaultValues).forEach(([key, value]) => {
        if (key === "dimensions" && value) {
          Object.entries(value).forEach(([dimKey, dimVal]) => {
            setValue(`dimensions.${dimKey}`, dimVal);
          });
        } else if (key === "discount_start_at" || key === "discount_end_at") {
          setValue(key, toDateInputValue(value));
        } else if (key === "tags") {
          // Convert ["hot", "new"] → ["hot", "new"] giữ nguyên, vì SelectBox xử lý multiple
          setValue("tags", value);
        } else if (key === "category_id") {
          // Ensure it's string if SelectBox dùng string compare
          setValue("category_id", String(value));
        } else {
          setValue(key, value);
        }
      });
